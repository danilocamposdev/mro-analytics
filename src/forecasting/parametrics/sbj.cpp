#include "mro/forecasting/parametrics/sbj.hpp"
...
namespace mro::forecasting::parametrics {
	...
	double SbjResult::demand_variance() const {
		// Var(c * X) = c^2 * Var(X), where c = (1 - alpha/(2-alpha)) and
		// Var(X) is Croston's underlying demand variance
		// (see CrostonResult::demand_variance()).
