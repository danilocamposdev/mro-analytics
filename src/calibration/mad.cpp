#include "mro/calibration/mad.hpp"
...
namespace mro::calibration {
	...
	namespace {
		...
		double mad_score(const std::vector<double>& errors) {
			double sum_abs_error = 0.0;
			for (double err : errors) {
				sum_abs_error += std::abs(err);
			}
			return sum_abs_error / static_cast<double>(errors.size());
		}
	...
	mro::calibration::types::SingleParamCalibResult calibrate_by_mad(
		...
		const auto [best_alpha, best_score] = mro::calibration::detail::grid_search_single_param(
				time_series, init, skip_periods, grid_step, forecast_fn, mad_score);
